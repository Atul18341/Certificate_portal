import React from "react";

// 🔵 Custom LinkedIn SVG Icon Component
const LinkedInIcon = () => (
  <svg
    className="w-4 h-4 fill-current"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.28 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.75M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z" />
  </svg>
);

interface LinkedInButtonProps {
  certificateTitle: string;
  organizationName: string;
  issueYear: number;
  issueMonth: number;
  certificateId: string;
  certificateUrl: string;
}

export default function LinkedInButton({
  certificateTitle,
  organizationName,
  issueYear,
  issueMonth,
  certificateId,
  certificateUrl,
}: LinkedInButtonProps) {
  const handleAddToLinkedIn = () => {
    const linkedInUrl = new URL("https://www.linkedin.com/profile/add");

    linkedInUrl.searchParams.append("startTask", "CERTIFICATION_NAME");
    linkedInUrl.searchParams.append("name", certificateTitle);
    linkedInUrl.searchParams.append("organizationName", organizationName);
    linkedInUrl.searchParams.append("issueYear", issueYear.toString());
    linkedInUrl.searchParams.append("issueMonth", issueMonth.toString());
    linkedInUrl.searchParams.append("certId", certificateId);
    linkedInUrl.searchParams.append("certUrl", certificateUrl);

    window.open(linkedInUrl.toString(), "_blank", "noopener,noreferrer");
  };

  return (
    <button
      onClick={handleAddToLinkedIn}
      className="px-4 py-2 bg-[#0A66C2] hover:bg-[#084e96] text-white font-bold text-xs rounded-xl flex items-center gap-2 shadow-md transition active:scale-95"
    >
      <LinkedInIcon />
      Add to LinkedIn Profile
    </button>
  );
}